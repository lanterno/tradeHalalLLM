...
class TradeRepo(Protocol):
    ...
    async def get_recently_closed(self, *, minutes: int = ...) -> list[dict[str, Any]]: ...
    async def get_recent_sells(self, *, minutes: int = ...) -> list[dict[str, Any]]: ...
...
class CryptoTradeRepo(Protocol):
    ...
    async def get_recent_crypto_trades(self, limit: int = ...) -> list[dict[str, Any]]: ...
...
class HalalCacheRepo(Protocol):
    ...
    async def is_crypto_cache_fresh(self, max_age_hours: int = ...) -> bool: ...
...
class StockHalalCacheRepo(Protocol):
    ...
    async def cache_halal_status(
        self, symbol: str, compliance: str, detail: str | None = ...
    ) -> None: ...
    async def is_cache_fresh(self, max_age_hours: int = ...) -> bool: ...
...
class DailyRecommendationRepo(Protocol):
...
class StrategyAdjustmentRepo(Protocol):
    ...
    async def get_recent_adjustments(self, limit: int = ...) -> list[dict[str, Any]]: ...
