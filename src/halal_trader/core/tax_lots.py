...
def _ordered_for_method(lots: Iterable[TaxLot], method: LotMethod) -> list[TaxLot]:
    ...
    if method is LotMethod.FIFO:
        return sorted(lots, key=lambda lot: (lot.acquisition_date, lot.lot_id))
    if method is LotMethod.LIFO:
        return sorted(lots, key=lambda lot: (lot.acquisition_date, lot.lot_id), reverse=True)
    if method is LotMethod.HIFO:
        return sorted(
            lots, key=lambda lot: (-lot.cost_basis_per_share, lot.acquisition_date, lot.lot_id)
        )
...
def apply_sale(
    ...
    *,
    ...
    total_open = sum(lot.quantity for lot in pool)
    ...
    new_lots.sort(key=lambda lot: (lot.acquisition_date, lot.lot_id))
    return tuple(realised), tuple(new_lots)


def total_quantity(pool: Iterable[TaxLot]) -> float:
    return sum(lot.quantity for lot in pool)


def total_cost_basis(pool: Iterable[TaxLot]) -> float:
    return sum(lot.quantity * lot.cost_basis_per_share for lot in pool)
