...
async def _engine_and_repo(tmp_path):
...
async def test_check_trade_triggers_stop_loss(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
...
async def test_check_trade_triggers_take_profit(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
        tid = await repo.record_trade(
...
async def test_check_trade_holds_inside_band(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
        tid = await repo.record_trade(
            symbol="AAPL", side="buy", quantity=10, price=200.0, stop_loss=190.0, target_price=220.0
...
async def test_trailing_stop_ratchets_up(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
        tid = await repo.record_trade(
            symbol="AAPL", side="buy", quantity=10, price=200.0, stop_loss=190.0, target_price=220.0
        ...
        async with engine.begin() as conn:
            row = await conn.execute(
                sa.text("SELECT stop_loss FROM trades WHERE id = :i"), {"i": tid}
            )
            new_sl = row.first()[0]
...
async def test_exit_skipped_when_alpaca_returns_error(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
        tid = await repo.record_trade(
            symbol="AAPL", side="buy", quantity=10, price=200.0, stop_loss=190.0, target_price=220.0
...
async def test_exit_swallows_mcp_exception(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
        tid = await repo.record_trade(
            symbol="AAPL", side="buy", quantity=10, price=200.0, stop_loss=190.0, target_price=220.0
...
async def test_exit_calls_retrainer_with_return_pct(tmp_path):
...
async def test_exit_swallows_retrainer_exception(tmp_path):
...
async def test_get_open_trades_returns_only_unclosed_buys(tmp_path):
    engine, repo = await _engine_and_repo(tmp_path)
    try:
